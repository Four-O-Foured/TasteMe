...
const RecipesUpdate = () => {
  ...
  const UpdateHandler = (res) => {
    const resIndex = data.findIndex((item) => item.id == id);
    
  
    
    const tempData = [...data];

    tempData[resIndex] = { ...tempData[resIndex], ...res };

    setData(tempData);

    localStorage.setItem("recipes", JSON.stringify(tempData));
    navigate(`/recipes/details/${id}`);
    toast.success("Recipe Updated", { autoClose: 1000 });
  };

  return (
    <div className="w-full mt-25 pl-20">
      <form onSubmit={handleSubmit(UpdateHandler)}>
        ...
        <button className="text-2xl p-2 px-5 bg-[#B85C38] rounded text-[#2D2424] font-semibold my-5 active:scale-95 hover:scale-105">
          Update
        </button>
      </form>
    </div>
